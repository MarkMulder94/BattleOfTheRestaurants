...
namespace Battle.Library.DataAcces.Koffie
{
    public class KoffieDataService
    {
        ...
        public List<KoffieBattle> LiveKoffieBattle()
        {
            ...
            {
                connection.Open();
                string sql = @"SELECT A.Groot, A.Medium, A.datum, A.NameManager, A.NameRestaurant 
                               FROM KoffieBattle A 
                               INNER JOIN(SELECT NameRestaurant, MAX(datum) as datum 
                               FROM KoffieBattle GROUP BY NameRestaurant)
